...
'
'  Filename: admOrganisationList.asp
'  Date:     $Date: 2004/03/11 06:08:42 $
'  Version:  $Revision: 1.6 $
'  Purpose:  Administration page for listing all Organisations
' ----------------------------------------------------------------------------------
%>
<%
Option Explicit
...
<%
  ...
  binUserPermMask = GetUserPermMask
	...
	objCollection.Query = "SELECT OrgPK, OrgName, OrgTypeFK, IsActive, PrimaryContactFK FROM tblOrganisations ORDER BY OrgPK ASC"
	...
	Else
		...
		Else
			...
			Do While Not objCollection.EOF And Application("ITEMS_PER_PAGE") > I
				...
				strHTML = strHTML & "	<TD class=LeftColumn align=left>&nbsp; " & objCollection.Item.OrgName & "</TD>" & Chr(13)
				strHTML = strHTML & "	<TD class=LeftColumn align=left>&nbsp;" & objCollection.Item.OrgType.ItemName & "</TD>" & Chr(13)
				strHTML = strHTML & "	<TD class=LeftColumn align=center>" & objCollection.Item.PrimaryContact.FName & " " & objCollection.Item.PrimaryContact.LName & "</TD>" & Chr(13)
				...
				End If
				strHTML = strHTML & "	<TD class=RightColumn align=center><A href=""admOrganisation.asp?ID=" & objCollection.Item.ID & """><IMG src=""Images/Pencil.gif"" alt=""Edit"" border=""0""></A></TD>" & Chr(13)
				strHTML = strHTML & "</TR>" & Chr(13)
    ...
    <TD>
		<TABLE class=Normal cellSpacing=1 cellPadding=1 width="100%" border=0 style="WIDTH: 100%" bgColor=white>
			<TR class="lhd_Heading1">
				<TD colspan=5 align=center><%=Lang("Organisations")%></TD>
   			</TR>
		    <TR class=ColumnHeading>
		    <TD class=LeftColumn align=left width="35%">&nbsp;<%=Lang("Organisation")%></TD>
		    <TD class=LeftColumn align=left width="15%">&nbsp;<%=Lang("Type")%></TD>
		    <TD class=LeftColumn align=Center width="25%"><%=Lang("Primary_Contact")%></TD>
...
<TR>
    <TD>
		<TABLE class=Normal cellSpacing=1 cellPadding=1 width="100%" border=0 bgColor=white>
			<%
			Response.Write DisplayPageNumbers(  "admOrganisationlist.asp?", intPage, intPages )
			%>
			<TR>
				<TD align=right>
					<BR><INPUT style="WIDTH: 80px; BACKGROUND-COLOR: white" OnClick="VBScript:window.navigate 'admOrganisation.asp'" id=btnNew name=btnNew type=submit value="<%=Lang("New")%>">
				</TD>
			</TR>
		</TABLE>
	</TD>
