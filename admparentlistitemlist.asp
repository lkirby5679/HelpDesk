...
'
'  Filename: admParentListItemList.asp
'  Date:     $Date: 2004/03/11 06:08:42 $
'  Version:  $Revision: 1.6 $
'  Purpose:  Administration page for listing all Parent Lists
' ----------------------------------------------------------------------------------
...
<%
  ...
  binUserPermMask = GetUserPermMask
	...
	objCollection.Query = "SELECT * FROM tblLists WHERE ParentListItemFK Is Null ORDER BY ItemOrder ASC"
	...
	Else
		...
		Else
			...
			Do While Not objCollection.EOF And Application("ITEMS_PER_PAGE") > I
				...
				End If
				strHTML = strHTML & "	<TD class=RightColumn align=center><A href=""admParentListItem.asp?ID=" & objCollection.Item.ID & """><IMG src=""Images/Pencil.gif"" alt=""Edit"" border=""0""></A>&nbsp;&nbsp;&nbsp;&nbsp;<A href=""admListItemList.asp?List=" & objCollection.Item.ID & "&Page=1""><IMG src=""Images/Manage.gif"" alt=""Manage List Items"" border=""0""></A></TD>" & Chr(13)
				strHTML = strHTML & "</TR>" & Chr(13)
    ...
    <TD>
		<TABLE class=Normal cellSpacing=1 cellPadding=1 width="100%" border=1 style="WIDTH: 100%" bgColor=white>
			<TR class="lhd_Heading1">
				<TD colspan=4 align=center><%=Lang("Lists")%></TD>
   			</TR>
		    <TR class=ColumnHeading>
		    <TD class=LeftColumn align=left width="35%">&nbsp;<%=Lang("List_Name")%></TD>
		    <TD class=LeftColumn align=center width="20%"><%=Lang("Item_Order")%></TD>
		    <TD class=LeftColumn align=center width="10%"><%=Lang("Active")%></TD>
		    <TD class=RightColumn align=center width="35%"><%=Lang("Options")%></TD>
		    </TR>
		    <%
...
<TR>
    <TD>
		<TABLE class=Normal cellSpacing=1 cellPadding=1 width="100%" border=0 bgColor=white>
			<%
			Response.Write DisplayPageNumbers(  "admParentListItemList.asp?", intPage, intPages )
			%>
			<TR>
				<TD align=right>
					<BR><INPUT style="WIDTH: 80px; BACKGROUND-COLOR: white" OnClick="VBScript:window.navigate 'admParentListItem.asp'" id=btnNew name=btnNew type=submit value="<%=Lang("New")%>">
				</TD>
			</TR>
		</TABLE>
	</TD>
