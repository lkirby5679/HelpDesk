...
'
'  Filename: admListItemList.asp
'  Date:     $Date: 2004/03/11 06:08:42 $
'  Version:  $Revision: 1.6 $
'  Purpose:  Administration page for listing a parent lists, list items
' ----------------------------------------------------------------------------------
%>
<%
Option Explicit
...
<%
	...
	Dim objCollection, objListItem
	Dim I, intPages, intPage, intUserID, intParentListItemID
	Dim strHTML, strParentListItem
	...
	intParentListItemID = CInt(Request.QueryString("List"))
	...
	objCollection.Query = "SELECT * FROM tblLists WHERE ParentListItemFK = " & intParentListItemID & " ORDER BY ItemOrder ASC"
	...
	Else
		...
		Else
			...
			Do While Not objCollection.EOF And Application("ITEMS_PER_PAGE") > I
				...
				strHTML = strHTML & "<TR class=""lhd_TableRow"">" & Chr(13)
				strHTML = strHTML & "	<TD class=LeftColumn align=left>&nbsp;" & objCollection.Item.ItemName & "</TD>" & Chr(13)
				strHTML = strHTML & "	<TD class=LeftColumn align=center>" & objCollection.Item.ItemOrder & "</TD>" & Chr(13)
				...
				End If
				strHTML = strHTML & "	<TD class=RightColumn align=center><A href=""admListItem.asp?ID=" & objCollection.Item.ID & """><IMG src=""Images/Pencil.gif"" alt=""Edit"" border=""0""></A></TD>" & Chr(13)
	...
	objListItem.ID = intParentListItemID
	
	If Not objListItem.Load Then
		' Item not found
	Else
		strParentListItem = objListItem.ItemName
	End If
...
<TABLE align=center cellSpacing=1 cellPadding=1 width="680" border=0>
  ...
  <TR>
    <TD>
		<TABLE class=Normal cellSpacing=1 cellPadding=1 width="100%" border=1 style="WIDTH: 100%" bgColor=white>
			<TR class="lhd_Heading1">
				<TD colspan=4 align=center><%=strParentListItem%>&nbsp;<%=Lang("Items")%></TD>
   			</TR>
		    <TR class=ColumnHeading>
		    <TD class=LeftColumn align=left width="55%">&nbsp;<%=Lang("Item_Name")%></TD>
		    <TD class=LeftColumn align=center width="20%"><%=Lang("Item_Order")%></TD>
...
<TR>
    <TD>
		<TABLE class=Normal cellSpacing=1 cellPadding=1 width="100%" border=0 bgColor=white>
			<%
			Response.Write DisplayPageNumbers(  "admListItemList.asp?", intPage, intPages )
			%>
			<TR>
				<TD align=right>
					<BR><INPUT style="WIDTH: 80px; BACKGROUND-COLOR: white" OnClick="VBScript:window.navigate 'admListItem.asp?List=<%=intParentListItemID%>'" id=btnNew name=btnNew type=submit value="<%=Lang("New")%>">
				</TD>
		    </TR>
		</TABLE>
	</TD>
