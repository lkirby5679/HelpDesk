...
'
'  Filename: admCaseTypeList.asp
'  Date:     $Date: 2004/03/11 06:08:42 $
'  Version:  $Revision: 1.6 $
'  Purpose:  Administration page for listing all Case Types
' ----------------------------------------------------------------------------------
%>
<%
Option Explicit
...
<%
  ...
  binUserPermMask = GetUserPermMask
	...
	objCollection.Query = "SELECT * FROM tblCaseTypes ORDER BY CaseTypeOrder ASC"
	...
	Else
		...
		Else
			...
			Do While Not objCollection.EOF And Application("ITEMS_PER_PAGE") > I
				...
				strHTML = strHTML & "	<TD class=LeftColumn align=left>&nbsp;" & objCollection.Item.CaseTypeName & "</TD>" & Chr(13)
				strHTML = strHTML & "	<TD class=LeftColumn align=left>&nbsp;" & objCollection.Item.CaseTypeDesc & "</TD>" & Chr(13)
				strHTML = strHTML & "	<TD class=LeftColumn align=center>&nbsp;" & objCollection.Item.CaseTypeOrder & "</TD>" & Chr(13)
				strHTML = strHTML & "	<TD class=LeftColumn align=left>&nbsp;" & objCollection.Item.RepGroup.GroupName & "</TD>" & Chr(13)
				...
				End If
				strHTML = strHTML & "	<TD class=RightColumn align=center><A href=""admCaseType.asp?ID=" & objCollection.Item.ID & """><IMG src=""Images/Pencil.gif"" alt=""Edit"" border=""0""></A></TD>" & Chr(13)
				strHTML = strHTML & "</TR>" & Chr(13)
...
<TABLE align=center cellSpacing=1 cellPadding=1 width="680" border=0>
	    ...
	    <TD>
			<TABLE class=Normal cellSpacing=1 cellPadding=1 width="100%" border=0 bgColor=white>
				<TR class="lhd_Heading1">
					<TD colspan=6 align=center><%=Lang("Case_Types")%></TD>
	   			</TR>
			    <TR class=ColumnHeading>
			        <TD class=LeftColumn align=left width="20%">&nbsp;<%=Lang("Case_Type")%></TD>
			        <TD class=LeftColumn align=left width="31%">&nbsp;<%=Lang("Description")%></TD>
			        <TD class=LeftColumn align=center width="7%"><%=Lang("Order")%></TD>
			        <TD class=LeftColumn align=left width="25%">&nbsp;<%=Lang("Rep_Group")%></TD>
			        <TD class=LeftColumn align=center width="7%"><%=Lang("Active")%></TD>
			        <TD class=RightColumn align=center width="10%"><%=Lang("Options")%></TD>
			    </TR>
			    <%
	...
	<TR>
	    <TD>
			<TABLE class=Normal cellSpacing=1 cellPadding=1 width="100%" border=0 bgColor=white>
			<%
			Response.Write DisplayPageNumbers(  "admCaseTypeList.asp?", intPage, intPages )
			%>
			<TR>
				<TD align=right>
					<BR><INPUT style="WIDTH: 80px; BACKGROUND-COLOR: white" OnClick="VBScript:window.navigate 'admCaseType.asp'" id=btnNew name=btnNew type=submit value="<%=Lang("New")%>">
				</TD>
			</TR>
			</TABLE>
		</TD>
