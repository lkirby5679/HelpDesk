...
'
'  Filename: admCategoryList.asp
'  Date:     $Date: 2004/03/11 06:08:42 $
'  Version:  $Revision: 1.6 $
'  Purpose:  Administration page for listing all Categories
' ----------------------------------------------------------------------------------
%>
<%
Option Explicit
...
<%
  ...
  binUserPermMask = GetUserPermMask
	...
	objCollection.Query = "SELECT CatPK, CatName, CatDesc, CatOrder, CaseTypeFK, IsActive FROM tblCategories ORDER BY CaseTypeFK, CatOrder ASC"
	...
	Else
		...
		Else
			...
			Do While Not objCollection.EOF And Application("ITEMS_PER_PAGE") > I
				...
				strHTML = strHTML & "	<TD class=LeftColumn align=left>&nbsp;" & objCollection.Item.CatName & "</TD>" & Chr(13)
				strHTML = strHTML & "	<TD class=LeftColumn align=center>&nbsp;" & objCollection.Item.CatOrder & "</TD>" & Chr(13)
				...
				End If
				strHTML = strHTML & "	<TD class=RightColumn align=center><A href=""admCategory.asp?ID=" & objCollection.Item.ID & """><IMG src=""Images/Pencil.gif"" alt=""Edit"" border=""0""></A></TD>" & Chr(13)
				strHTML = strHTML & "</TR>" & Chr(13)
    ...
    <TD>
		<TABLE class=Normal cellSpacing=1 cellPadding=1 width="100%" border=0 bgColor=white>
			<TR class="lhd_Heading1">
				<TD colspan=5 align=center><%=Lang("Categories")%></TD>
   			</TR>
		    <TR class=ColumnHeading>
		    <TD class=LeftColumn align=left width="30%">&nbsp;<%=Lang("Case_Type")%></TD>
		    <TD class=LeftColumn align=left width="30%">&nbsp;<%=Lang("Category")%></TD>
		    <TD class=LeftColumn align=center width="15%"><%=Lang("Order")%></TD>
...
<TR>
    <TD>
		<TABLE class=Normal cellSpacing=1 cellPadding=1 width="100%" border=0 bgColor=white>
			<%
			Response.Write DisplayPageNumbers(  "admCategoryList.asp?", intPage, intPages )
			%>
		    <TR>
				<TD align=right>
					<BR><INPUT style="WIDTH: 80px; BACKGROUND-COLOR: white" OnClick="VBScript:window.navigate 'admCategory.asp'" id=btnNew name=btnNew type=submit value="<%=Lang("New")%>">
				</TD>
		    </TR>
		</TABLE>
	</TD>
