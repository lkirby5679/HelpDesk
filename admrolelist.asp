...
'
'  Filename: admRoleList.asp
'  Date:     $Date: 2004/03/11 06:08:42 $
'  Version:  $Revision: 1.6 $
'  Purpose:  Administration page for listing all Roles
' ----------------------------------------------------------------------------------
%>
<%
Option Explicit
...
<%
  ...
  ' Get and validate user variables
  ...
  intUserID = GetUserID
	...
	objCollection.Query = "SELECT RolePK, RoleName, RoleDesc, IsActive FROM tblRoles ORDER BY RolePK ASC"
	...
	Else
		...
		Else
			...
			Do While Not objCollection.EOF And Application("ITEMS_PER_PAGE") > I
				...
				strHTML = strHTML & "	<TD class=LeftColumn align=left>&nbsp;" & objCollection.Item.RoleName & "</TD>" & Chr(13)
				strHTML = strHTML & "	<TD class=LeftColumn align=left>&nbsp;" & objCollection.Item.RoleDesc & "</TD>" & Chr(13)
				...
				End If
				strHTML = strHTML & "	<TD class=RightColumn align=center><A href=""admRole.asp?ID=" & objCollection.Item.ID & """><IMG src=""Images/Pencil.gif"" alt=""Edit"" border=""0""></A></TD>" & Chr(13)
				strHTML = strHTML & "</TR>" & Chr(13)
...
<TABLE align=center cellSpacing=1 cellPadding=1 width="680" border=0>
  ...
  <TR>
    <TD>
		<TABLE class=Normal cellSpacing=1 cellPadding=1 width="100%" border=1 bgColor=white>
			<TR class="lhd_Heading1">
				<TD colspan=4 align=center><%=Lang("Roles")%></TD>
   			</TR>
		    <TR class=ColumnHeading>
		    <TD class=LeftColumn align=left width="25%">&nbsp;<%=Lang("Role")%></TD>
		    <TD class=LeftColumn align=left width="50%">&nbsp;<%=Lang("Description")%></TD>
...
<TR>
    <TD>
		<TABLE class=Normal cellSpacing=1 cellPadding=1 width="100%" border=0 bgColor=white>
			<%
			Response.Write DisplayPageNumbers(  "admRoleList.asp?", intPage, intPages )
			%>
		    <TR>
				<TD align=right>
					<BR><INPUT style="WIDTH: 80px; BACKGROUND-COLOR: white" OnClick="VBScript:window.navigate 'admRole.asp'" id=btnNew name=btnNew type=submit value="<%=Lang("New")%>">
				</TD>
		    </TR>
		</TABLE>
	</TD>
