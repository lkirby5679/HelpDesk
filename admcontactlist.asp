...
'
'  Filename: admContactList.asp
'  Date:     $Date: 2004/03/29 08:21:29 $
'  Version:  $Revision: 1.7 $
'  Purpose:  Administration page for listing all Contacts
' ----------------------------------------------------------------------------------
%>
<%
Option Explicit
...
<%
  ...
  binUserPermMask = GetUserPermMask
	...
	objCollection.Query = "SELECT ContactPK, DeptFK, FName, LName, Username, IsActive FROM tblContacts ORDER BY Username ASC"
	...
	Else
		...
		Else
			...
			Do While Not objCollection.EOF And Application("ITEMS_PER_PAGE") > I
				...
				strHTML = strHTML & "	<TD class=LeftColumn align=center>" & objCollection.Item.UserName & "</TD>" & Chr(13)
				strHTML = strHTML & "	<TD class=LeftColumn align=left>&nbsp;" & objCollection.Item.FName & " " & objCollection.Item.LName & "</A></TD>" & Chr(13)
				strHTML = strHTML & "	<TD class=LeftColumn align=left><A href=""admDepartment.asp?ID=" & objCollection.Item.DeptID & """>&nbsp;" & objCollection.Item.Dept.DeptName & "</A></TD>" & Chr(13)
				...
				End If
				strHTML = strHTML & "	<TD class=RightColumn align=center><A href=""admContact.asp?Mode=3&ID=" & objCollection.Item.ID & """><IMG src=""Images/Pencil.gif"" alt=""Edit"" border=""0""></A></TD>" & Chr(13)
				strHTML = strHTML & "</TR>" & Chr(13)
    ...
    <TD>
		<TABLE class=Normal cellSpacing=1 cellPadding=1 width="100%" border=1 style="WIDTH: 100%" bgColor=white>
			<TR class="lhd_Heading1">
				<TD colspan=5 align=center><%=Lang("Contacts")%></TD>
   			</TR>
		    <TR class=ColumnHeading>
		    <TD class=LeftColumn align=center width="15%"><%=Lang("User_Name")%></TD>
		    <TD class=LeftColumn align=left width="35%">&nbsp;<%=Lang("Full_Name")%></TD>
		    <TD class=LeftColumn align=left width="25%">&nbsp;<%=Lang("Department")%></TD>
...
<TR>
    <TD>
		<TABLE class=Normal cellSpacing=1 cellPadding=1 width="100%" border=0 bgColor=white>
			<%
			Response.Write DisplayPageNumbers(  "admContactList.asp?", intPage, intPages )
			%>
		    <TR>
				<TD align=right>
					<BR><INPUT style="WIDTH: 80px; BACKGROUND-COLOR: white" OnClick="VBScript:window.navigate 'admContact.asp'" id=btnNew name=btnNew type=submit value="<%=Lang("New")%>">
				</TD>
		    </TR>
		</TABLE>
	</TD>
